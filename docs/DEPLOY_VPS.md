...
|---|---|---|
| `ddag.example.com` | Static Vue 3 + Vite dashboard, admin control-plane API, OAuth token API | Caddy serves `apps/dashboard/dist`; admin-backend `127.0.0.1:8080`, auth-service `127.0.0.1:8081` |
...
cd apps/dashboard
sudo -u ddag corepack enable
sudo -u ddag pnpm install --frozen-lockfile
sudo -u ddag pnpm run build
```
...
# The static Vue 3 + Vite dashboard is served by Caddy from apps/dashboard/dist.
# Same-origin paths are compiled into the bundle; no frontend runtime env is needed.
```
...
Build the static Vue 3 + Vite dashboard once per release. Caddy serves
`/opt/ddag/apps/dashboard/dist` directly, so no Node or dashboard systemd unit
runs in production:

```bash
cd /opt/ddag/apps/dashboard
sudo -u ddag corepack enable
sudo -u ddag pnpm install --frozen-lockfile
sudo -u ddag pnpm run build
```

Reload and start the backend services:
...
ddag.example.com {
  ...
  root * /opt/ddag/apps/dashboard/dist
...
DDAG's metadata database is protected by the encrypted logical-backup and
restore-drill timers documented in the [Backup, Recovery, and PITR
Runbook](BACKUP_RECOVERY.md). Do not maintain a second, ad-hoc backup procedure
alongside those controls.

Before an upgrade, verify the most recent successful backup and record the
release SHA being deployed:

```bash
sudo systemctl status ddag-logical-backup.service --no-pager
sudo journalctl -u ddag-logical-backup.service -n 20 --no-pager
cd /opt/ddag && git rev-parse HEAD
```

The following are deployment secrets and recovery material, not repository
files. Store them independently with restricted access:

- the application environment file;
- `/etc/ddag/backup-runner.env`, including the backup encryption key; and
- encrypted backup artifacts plus their manifests.

Redis holds cache and rate-limit counters only. It can be rebuilt; persistence
can reduce disruption during restarts.

For a replacement host, install DDAG and its backup units, restore the secret
material through the approved secret process, copy encrypted artifacts from an
independent destination, then run an isolated restore drill before production
cutover. The detailed sequence and acceptance evidence are in
[Backup, Recovery, and PITR Runbook](BACKUP_RECOVERY.md).
