...
build: ## Build all service binaries into ./bin
	@mkdir -p $(BIN)
	@for s in $(SERVICES); do echo "build $$s"; $(GO) build -o $(BIN)/ddag-$$s ./cmd/$$s || exit 1; tmp=$$(mktemp $(BIN)/.$$s.XXXXXX); install -m 0755 $(BIN)/ddag-$$s $$tmp; mv -f $$tmp $(BIN)/$$s; done
...
.PHONY: dashboard
dashboard: ## Run the Vite dashboard dev server (:3000)
	@cd apps/dashboard && pnpm install && pnpm dev --port 3000
