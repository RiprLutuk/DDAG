...
type Config struct {
	...
	GRPCAddr string // optional private connector gRPC listener; empty disables it
...
type GatewayConfig struct {
	...
	ConnectorURLs       map[string]string // db_type -> connector HTTP base URL
	ConnectorGRPCURLs   map[string]string // db_type -> optional connector gRPC address; empty keeps HTTP transport
...
func Load(service string) Config {
	c := Config{
		...
		GRPCAddr: getEnv("DDAG_GRPC_ADDR", ""), // connector-only; empty keeps gRPC listener disabled
		...
		Gateway: GatewayConfig{
			...
			// Empty by default: this is an explicit, canary-safe opt-in per DB type.
			ConnectorGRPCURLs: map[string]string{
				"postgres":  getEnv("DDAG_CONNECTOR_POSTGRES_GRPC_ADDR", ""),
				"mysql":     getEnv("DDAG_CONNECTOR_MYSQL_GRPC_ADDR", ""),
				"oracle":    getEnv("DDAG_CONNECTOR_ORACLE_GRPC_ADDR", ""),
				"sqlserver": getEnv("DDAG_CONNECTOR_SQLSERVER_GRPC_ADDR", ""),
			},
		...
		ConnectorHealthInterval: getEnvDuration("DDAG_CONNECTOR_HEALTH_INTERVAL", time.Minute),
	}
	return c
}
...
func (c Config) Validate() error {
	...
	internalSecret := strings.TrimSpace(c.Gateway.InternalAuthSecret)
	if internalSecret == "" || strings.HasPrefix(internalSecret, "REPLACE_WITH") {
		problems = append(problems, "DDAG_INTERNAL_AUTH_SECRET must be set in production")
	}
	if len(problems) > 0 {
