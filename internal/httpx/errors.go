...
var statusByCode = map[string]int{
	...
	CodeDBQueryTimeout:        http.StatusGatewayTimeout,
...
func Error(w http.ResponseWriter, r *http.Request, err error) {
	...
	if apiErr.Code == CodeUnauthorized {
		w.Header().Set("WWW-Authenticate", `Bearer realm="DDAG API"`)
	}
...
// ErrorCodeWithDetails writes an error envelope from a code, message, and details directly.
func ErrorCodeWithDetails(w http.ResponseWriter, r *http.Request, code, message string, details map[string]any) {
	Error(w, r, NewErrorWithDetails(code, message, details))
}
