...
func Run() error {
	...
	if cfg.ConnectorHealthInterval > 0 {
		go svc.connectorHealthLoop(healthCtx, cfg.ConnectorHealthInterval)
		go svc.serviceHealthLoop(healthCtx, cfg.ConnectorHealthInterval)
	}

	return server.Service{
		Name: "admin-backend", Addr: cfg.HTTPAddr, Handler: svc.routes(), Logger: log, Metrics: m,
		Ready:      func() bool { return pool.Ping(ctx) == nil },
		OnShutdown: func(context.Context) { cancelHealth(); _ = rdb.Close(); pool.Close() },
	}.Run()
}
