...
func (s *service) updateConnection(w http.ResponseWriter, r *http.Request) {
	...
	if in.Password != "" {
		newRef, err := storeConnectionPassword(r.Context(), s.secrets, existing, in.Password)
		if err != nil {
			httpx.ErrorCode(w, r, httpx.CodeInternal, "failed to store secret")
			return
		}
		if newRef != nil {
			if err := s.store.SetConnectionSecret(r.Context(), id, *newRef); err != nil {
				_ = s.secrets.Delete(r.Context(), *newRef)
				storeErr(w, r, err)
				return
			}
		}
...
func storeConnectionPassword(ctx context.Context, secrets secret.Store, existing *models.DatabaseConnection, password string) (*uuid.UUID, error) {
	if password == "" {
		return nil, nil
	}
	if existing.SecretRef != nil {
		if err := secrets.Update(ctx, *existing.SecretRef, []byte(password)); err != nil {
			return nil, err
		}
		return nil, nil
	}
	ref, err := secrets.Put(ctx, []byte(password), "db_password")
	if err != nil {
		return nil, err
	}
	return &ref, nil
...
func (s *service) testConnection(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		base := s.connectors[in.DatabaseType]
		msg := fmt.Sprintf("connector unavailable for %s (check if service is running at %s)", in.DatabaseType, base)
		httpx.ErrorCode(w, r, httpx.CodeConnectorError, msg)
		return
	}
...
func (s *service) testSavedConnection(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		_ = s.store.SetConnectionHealth(r.Context(), id, connectionHealthStatus(nil, err))
		base := s.connectors[c.DatabaseType]
		msg := fmt.Sprintf("connector unavailable for %s (check if service is running at %s)", c.DatabaseType, base)
		httpx.ErrorCode(w, r, httpx.CodeConnectorError, msg)
		return
	}
	status := connectionHealthStatus(res, nil)
...
func connectionHealthStatus(res map[string]any, err error) string {
	if err != nil {
		return "unreachable"
	}
...
func (s *service) callConnectorTest(ctx context.Context, dbType string, payload map[string]any) (map[string]any, error) {
	...
	if !found || base == "" {
		return map[string]any{"success": false, "message": "no connector configured for " + dbType}, nil
	}
