...
func (s *service) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// The session token is deliberately cookie-only so browser JavaScript cannot
	// exfiltrate it through an XSS bug. The HttpOnly session cookie above is the
	// sole browser credential; callers receive only non-sensitive session data.
