...
func (s *service) listRateLimits(w http.ResponseWriter, r *http.Request) {
	p := listParams(r)
	rules, total, err := s.store.ListRateLimitRulesPage(r.Context(), p)
	if err != nil {
		storeErr(w, r, err)
		return
	}
	list(w, r, rules, p, total)
}
...
func (s *service) listIPWhitelists(w http.ResponseWriter, r *http.Request) {
	p := listParams(r)
	entries, total, err := s.store.ListIPWhitelistsPage(r.Context(), p)
	if err != nil {
		storeErr(w, r, err)
		return
	}
	list(w, r, entries, p, total)
}
...
func (s *service) listCacheRules(w http.ResponseWriter, r *http.Request) {
	p := listParams(r)
	rules, total, err := s.store.ListCacheRulesPage(r.Context(), p)
	if err != nil {
		storeErr(w, r, err)
		return
	}
