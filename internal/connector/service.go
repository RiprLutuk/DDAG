...
type service struct {
	...
	replayStore        internalauth.ReplayStore
...
func Run(dbType string) error {
	...
	var replayStore internalauth.ReplayStore
	if cfg.Gateway.InternalAuthSecret != "" {
		if cfg.Redis.Addr == "" {
			return fmt.Errorf("redis is required when internal authentication is enabled for replay protection")
		}
		rdb := redis.NewClient(&redis.Options{Addr: cfg.Redis.Addr, Password: cfg.Redis.Password, DB: cfg.Redis.DB})
		if err := rdb.Ping(context.Background()).Err(); err != nil {
			return fmt.Errorf("connect redis replay store: %w", err)
		}
		replayStore = internalauth.NewRedisReplayStore(rdb)
	}
	...
	svc := &service{
		...
		breakers:  map[string]*circuit.Breaker{},
		admission: newAdmissionController(cfg.Gateway.BackpressureSize, cfg.Gateway.BackpressureTimeout),
	}
	...
	if cfg.GRPCAddr != "" {
		...
		connectorv1.RegisterConnectorServiceServer(grpcSrv, newGRPCServer(svc))
		go func() {
			if err := grpcSrv.Serve(grpcListener); err != nil {
				log.Error("grpc_server_stopped", "error", err.Error())
			}
		}()
		log.Info("grpc_listener_started", "addr", cfg.GRPCAddr)
	}
...
func (s *service) handleQuery(w http.ResponseWriter, r *http.Request) {
	...
	// Admission is intentionally enforced at the connector, immediately before
	// pool/query work. Gateway replicas may scale horizontally, but this guard
	// remains the last line of defence for this source DB on every connector.
	release, admitted := s.admission.Acquire(r.Context(), rc.idStr, rc.cfg.MaxPool)
	s.observeAdmission(rc.idStr)
	if !admitted {
		s.metrics.AdmissionRejected.WithLabelValues(rc.idStr, s.dbType).Inc()
		s.metrics.ConnectorErr.WithLabelValues(rc.idStr, s.dbType).Inc()
		s.writeErr(w, http.StatusServiceUnavailable, httpx.CodeBackpressureLimit, "Database is busy. Please retry later.", string(breaker.State()))
		return
	}
	defer func() { release(); s.observeAdmission(rc.idStr) }()
...
func (s *service) handleCircuits(w http.ResponseWriter, r *http.Request) {
	if s.internalAuthSecret != "" {
		if err := internalauth.VerifyHeadersWithReplayStore(r.Context(), r, nil, s.internalAuthSecret, time.Now(), time.Minute, s.replayStore); err != nil {
...
func (s *service) readAuthenticatedBody(w http.ResponseWriter, r *http.Request) ([]byte, bool) {
	...
	if err := internalauth.VerifyHeadersWithReplayStore(r.Context(), r, body, s.internalAuthSecret, time.Now(), time.Minute, s.replayStore); err != nil {
		s.writeErr(w, http.StatusUnauthorized, httpx.CodeUnauthorized, "invalid internal service signature")
		return nil, false
	}
	return body, true
...
func (s *service) observeAdmission(connectionID string) {
	snap := s.admission.Snapshot(connectionID)
	s.metrics.AdmissionInflight.WithLabelValues(connectionID, s.dbType).Set(float64(snap.Inflight))
	s.metrics.AdmissionWaiting.WithLabelValues(connectionID, s.dbType).Set(float64(snap.Waiting))
}
...
func classifyQueryErr(err error) (code string, status int) {
	...
	case strings.Contains(msg, "context deadline exceeded"), strings.Contains(msg, "timeout"):
		return httpx.CodeDBQueryTimeout, http.StatusGatewayTimeout
