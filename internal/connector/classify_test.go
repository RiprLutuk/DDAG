...
func TestClassifyQueryErr_QueryTimeoutUsesGatewayTimeout(t *testing.T) {
	code, status := classifyQueryErr(errors.New("context deadline exceeded"))
	if code != httpx.CodeDBQueryTimeout || status != http.StatusGatewayTimeout {
		t.Fatalf("got (%s, %d), want (%s, 504)", code, status, httpx.CodeDBQueryTimeout)
	}
}
