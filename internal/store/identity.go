...
func (s *Store) ListRoles(ctx context.Context) ([]models.Role, error) {
	...
	if err := s.selectRows(ctx, &roles,
		`SELECT id, name, description, is_system, created_at, updated_at FROM roles ORDER BY name`); err != nil {
		return nil, err
	}
	for i := range roles {
		var perms []string
...
func (s *Store) ListSettings(ctx context.Context) ([]models.Setting, error) {
	...
	err := s.selectRows(ctx, &out, `SELECT key, value, updated_by, updated_at FROM settings ORDER BY key`)
	return out, err
}

func (s *Store) GetSetting(ctx context.Context, key string) (*models.Setting, error) {
	var st models.Setting
	err := s.get(ctx, &st, `SELECT key, value, updated_by, updated_at FROM settings WHERE key=$1`, key)
	return &st, err
}
