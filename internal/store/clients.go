...
func (s *Store) ListScopes(ctx context.Context) ([]models.Scope, error) {
	out, _, err := s.ListScopesPage(ctx, ListParams{Page: 1, Limit: 200})
	return out, err
}

func (s *Store) ListScopesPage(ctx context.Context, p ListParams) ([]models.Scope, int64, error) {
	p.Normalize()
	var total int64
	if err := s.get(ctx, &total, scopeListCountSQL(), p.Search); err != nil {
		return nil, 0, err
	}
	var out []models.Scope
	err := s.selectRows(ctx, &out, scopeListSQL(p), p.Search, p.Limit, p.Offset())
...
func (s *Store) ListClients(ctx context.Context, p ListParams) ([]models.Client, int64, error) {
	...
	if err := s.selectRows(ctx, &clients,
		`SELECT `+clientCols+` FROM clients
		 ...
		 ORDER BY `+p.OrderBy(map[string]string{"client_id": "client_id", "name": "client_name", "environment": "environment", "status": "status", "created_at": "created_at"}, "created_at")+` LIMIT $2 OFFSET $3`, p.Search, p.Limit, p.Offset()); err != nil {
		return nil, 0, err
	}
	for i := range clients {
...
func consumeRefreshTokenSQL() string {
	return `UPDATE refresh_tokens SET revoked=true
		WHERE token_hash=$1 AND revoked=false AND expires_at > now()
		RETURNING id, token_hash, client_id, scope, expires_at, revoked, created_at`
}

// ConsumeRefreshToken atomically revokes one active refresh token and returns it.
// Concurrent requests using the same token cannot both succeed.
func (s *Store) ConsumeRefreshToken(ctx context.Context, hash string) (*models.RefreshToken, error) {
	var rt models.RefreshToken
	err := s.get(ctx, &rt, consumeRefreshTokenSQL(), hash)
	return &rt, err
}
