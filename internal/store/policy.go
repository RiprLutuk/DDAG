...
func (s *Store) ListCacheRules(ctx context.Context) ([]models.CacheRule, error) {
	out, _, err := s.ListCacheRulesPage(ctx, ListParams{Page: 1, Limit: 200})
	return out, err
}

func (s *Store) ListCacheRulesPage(ctx context.Context, p ListParams) ([]models.CacheRule, int64, error) {
	p.Normalize()
	var total int64
	if err := s.get(ctx, &total, cacheRuleListCountSQL(), p.Search); err != nil {
		return nil, 0, err
	}
	var out []models.CacheRule
	err := s.selectRows(ctx, &out, cacheRuleListSQL(p), p.Search, p.Limit, p.Offset())
...
func (s *Store) ListRateLimitRules(ctx context.Context) ([]models.RateLimitRule, error) {
	out, _, err := s.ListRateLimitRulesPage(ctx, ListParams{Page: 1, Limit: 200})
	return out, err
}

func (s *Store) ListRateLimitRulesPage(ctx context.Context, p ListParams) ([]models.RateLimitRule, int64, error) {
	p.Normalize()
	var total int64
	if err := s.get(ctx, &total, rateLimitListCountSQL(), p.Search); err != nil {
		return nil, 0, err
	}
	var out []models.RateLimitRule
	err := s.selectRows(ctx, &out, rateLimitListSQL(p), p.Search, p.Limit, p.Offset())
	return out, total, err
}
...
func (s *Store) ListIPWhitelists(ctx context.Context) ([]models.IPWhitelist, error) {
	out, _, err := s.ListIPWhitelistsPage(ctx, ListParams{Page: 1, Limit: 200})
	return out, err
}

func (s *Store) ListIPWhitelistsPage(ctx context.Context, p ListParams) ([]models.IPWhitelist, int64, error) {
	p.Normalize()
	var total int64
	if err := s.get(ctx, &total, ipWhitelistListCountSQL(), p.Search); err != nil {
		return nil, 0, err
	}
	var out []models.IPWhitelist
	err := s.selectRows(ctx, &out, ipWhitelistListSQL(p), p.Search, p.Limit, p.Offset())
	return out, total, err
}
