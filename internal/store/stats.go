...
func (s *Store) Overview(ctx context.Context) (*Overview, error) {
	...
	// Operational error rate excludes synthetic QA traffic and client-side 4xx
	// responses. It measures gateway/upstream availability (5xx) for real traffic.
	_ = s.get(ctx, &o.ErrorRateToday, `
		SELECT COALESCE(
			100.0 * count(*) FILTER (WHERE status_code >= 500) / NULLIF(count(*),0), 0)
		FROM api_request_logs
		WHERE created_at >= date_trunc('day', now())
		  AND client_label <> 'qa-tester'`)
	...
	o.TopSlow = topSlow
	topErr, err := s.endpointStats(ctx, `HAVING count(*) FILTER (WHERE status_code>=500) > 0 ORDER BY error_rate DESC`)
	if err != nil {
		return nil, err
	}
	o.TopErrors = topErr
...
func (s *Store) endpointStats(ctx context.Context, tail string) ([]EndpointStat, error) {
	...
	err := s.selectRows(ctx, &out, `
		SELECT
			...
			COALESCE(100.0 * count(*) FILTER (WHERE status_code>=500) / NULLIF(count(*),0),0) AS error_rate
		FROM api_request_logs
