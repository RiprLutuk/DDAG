...
func (s *service) refresh(w http.ResponseWriter, r *http.Request, refreshToken string) {
	...
	rt, err := s.store.ConsumeRefreshToken(r.Context(), auth.HashToken(refreshToken))
	if err != nil {
		s.metrics.TokenFailed.Inc()
	...
	// Rotate: the used refresh token was atomically consumed above; issue a fresh pair.
