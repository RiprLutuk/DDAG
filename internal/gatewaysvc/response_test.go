...
func TestWriteCachedRejectsCorruptPayloadWithoutWritingSuccess(t *testing.T) {
	rec := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/", nil)
	s := &service{}

	if ok := s.writeCachedWithTTL(rec, req, []byte(`not-json`), time.Now(), time.Minute); ok {
		t.Fatal("corrupt cache payload must not be accepted")
	}
	if rec.Body.Len() != 0 {
		t.Fatalf("corrupt cache wrote a false-success response: %s", rec.Body.String())
	}
}
