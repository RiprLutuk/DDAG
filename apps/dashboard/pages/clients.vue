...
async function load() {
  loading.value = true
  try {
    rows.value = (await api.list('/api/clients')).items
...
<template>
  ...
  <div class="toolbar"><div class="spacer" /><button class="btn primary" @click="openCreate">+ New Client</button></div>

  <UiTable :columns="columns" :rows="rows" :loading="loading" has-actions empty="No clients yet.">
