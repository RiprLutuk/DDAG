...
async function load() {
  ...
  try {
    const [rl, cl, ap] = await Promise.all([
      api.list(`/api/rate-limits?${new URLSearchParams({ page: String(table.page), limit: String(table.limit), search: table.search, sort_by: table.sortBy, sort_dir: table.sortDir })}`),
      api.list('/api/clients'),
      api.list('/api/apis')
    ])
    rows.value = rl.items; table.total = rl.pagination.total
...
function openEdit(row: any) {
  ...
  Object.assign(form, {
    ...
    applies_to: row.applies_to || 'client',
    requests_per_second: row.requests_per_second || 0,
    requests_per_minute: row.requests_per_minute || 0,
    requests_per_hour: row.requests_per_hour || 0,
    requests_per_day: row.requests_per_day || 0
  })
  modalOpen.value = true
}
