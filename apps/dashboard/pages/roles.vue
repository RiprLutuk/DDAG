...
definePageMeta({ title: 'Roles & Permissions' })
const api = useApi()
...
async function load() {
  loading.value = true
  try {
    rows.value = (await api.list('/api/roles')).items
...
<template>
  ...
  <div class="toolbar"><div class="spacer" /><button class="btn primary" @click="openCreate">+ New Role</button></div>

  <UiTable :columns="columns" :rows="rows" :loading="loading" has-actions empty="No roles defined yet.">
  ...
  <UiModal :open="editOpen" :title="`Permissions — ${editing?.name}`" wide @close="editOpen = false">
    ...
    <div class="field">
      ...
      <div class="row" style="flex-wrap:wrap;gap:8px">
        <label v-for="p in catalog" :key="p.id" class="checkbox" style="flex:0 0 calc(50% - 8px);align-items:flex-start">
